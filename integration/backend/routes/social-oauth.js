  ...
  try {
    ...
    if (platform === 'twitter') {
      ...
      const r = await fetch(config.tokenUrl, {
        ...
        body: new URLSearchParams({ client_id: config.clientId, client_secret: config.clientSecret, code, grant_type: 'authorization_code', redirect_uri: config.redirectUri, code_verifier: pkceVerifier }),
      });
