...
6. הצג את הנתונים מ-`platforms[platform_name]`: YouTube → מנויים + טבלת סרטונים (כותרת/צפיות/לייקים), Facebook → עוקבים + engagement לכל פוסט, Twitter → עוקבים + impressions לכל ציוץ, LinkedIn → עוקבי הדף
7. אם אין נתוני engagement מפלטפורמה — ציין שהסיבה היא הרשאות API מוגבלות, ולא שהפלטפורמה לא מחוברת
8. **חשוב**: אם `posts_tracked` = 0 זה לא אומר שלא פורסמו פוסטים אי-פעם. **אל תאמר** "נתחיל לפרסם" / "פוסט ראשון" — הצע במקום לוח תוכן או ניתוח אסטרטגי
