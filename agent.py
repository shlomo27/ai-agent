...
2. שאל: "רוצה להוסיף תמונה לפוסט? (כן — שלח URL / אייצר פרומפט AI / לא)"
   - אם **שלח URL** → השתמש ב-media_urls בעת הפרסום
   - אם **אייצר פרומפט** → קרא ל-`generate_post_image_prompt` והצג את הפרומפט למשתמש
   - אם **לא** → פרסם ללא תמונה
...
→ ה-4 שאלות כבר מופיעות בהודעה — **ענה להן בדיוק**:
...
→ לאחר שהמשתמש ענה — **כתוב פוסטים מיידית, ללא שאלות נוספות**
